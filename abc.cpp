...
std::vector<double> rouletteSelection(std::vector< std::vector<double> > &solutionPopulation, std::vector<double> fitnesses)//, std::string minOrMax)
 ...
 */
{
	...
	double random = static_cast <double> (rand() /( static_cast <double> (RAND_MAX/(probabilityRanges[probabilityRanges.size()-1][1]))));
	...
	{
		//DEBUG? std::cout << random << "  lower bound: " << probabilityRanges[i][0] << "  upper bound: " << probabilityRanges[i][1] << "\n" : 0;
...
std::vector<double> abcAlgorithm(std::string fitnessFunctionName, double searchSpaceLowerBound, double searchSpaceUpperBound, int dimensions, std::string minOrMax)
 ...
 */
{
	...
	std::vector<double> bestSolution = employedBees[0]; //give best solution an initial value
	...
	while(iter < 100) //tentative aspiration critera
	{
		...
		for(int eB = 0; eB < numberOfEmployedBees; eB++)
		{
			...
			if (employedBees[eB][randomDimension] > searchSpaceUpperBound)
			{
				employedBees[eB][randomDimension] = searchSpaceUpperBound;
			}
			else if (employedBees[eB][randomDimension] < searchSpaceLowerBound)
			{
				employedBees[eB][randomDimension] = searchSpaceLowerBound;
			}
		}
		...
		for(int eB = 0; eB < employedBees.size(); eB++)
		{
			if (minOrMax == "max")
			{
				if (fitnessFunctionName == "Ackley")
				{
					...
					{
						printf("bestSolution changed at iteration %d, bestsolution = %f\n\n", iter, Ackley(bestSolutionArray2));
				...
				else if (fitnessFunctionName == "Rastrigin")
				{
					...
					{
						printf("bestSolution changed at iteration %d, bestsolution = %f\n\n", iter, Rastrigin(bestSolutionArray2));
						bestSolution = employedBees[eB];
					}
				}
				else if (fitnessFunctionName == "Sphere")
				{
					...
					{
						printf("bestSolution changed at iteration %d, bestsolution = %f\n\n", iter, Sphere(bestSolutionArray10));
						bestSolution = employedBees[eB];
					}
				}
				else if (fitnessFunctionName == "SumOfPowers")
				{
					...
					{
						printf("bestSolution changed at iteration %d, bestsolution = %f\n\n", iter, SumOfPowers(bestSolutionArray10));
				 		bestSolution = employedBees[eB];
					}
				}
			}
...
//Calls all of our optimizations and displays their results.
{
	...
	std::vector<double> easomOptimized = abcAlgorithm("Easom", -100.0, 100.0, 2, "min");
	double easomArray[2];
